...
const displayComments = (comments) => {
    commentsCtnr.innerHTML = comments.map(comments=>{
        return asComment(comments)
    }).join('');
}
...
function asComment(c) {
    let commentHtml = `<div id="commentCntr-${c.commentId}" class= "forum">`

    commentHtml += `<h3>${c.user}  ${c.created}</h3>`
    commentHtml += `<p class="forum-text">${c.message}</p>`
    commentHtml += `</div>`
